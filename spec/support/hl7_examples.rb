class HL7Examples
  def self.a01
    str = <<~EOS
    MSH|^~\\&|SOURCEEHR|WA|MIRTHDST|WA|201611111111||ADT^A01|MSGID10001|P|2.3|
    EVN|A01|201611111111||
    PID|1|100001^^^1^MRN1|900001~555555||DOE&DE^JOHN^^^^||19601111|M||WH|111 THAT PL^^HERE^WA^98020^USA||(206)555-5309|||M|NON|999999999|
    NK1|1|DOE^JANE^|WIFE||(206)555-5555||||NK^NEXT OF KIN
    NK1|2|DOE^JONAH^|BRO||(206)555-1515||||NK^NEXT OF KIN
    PV1|1|O|1001^2002^01||||123456^DOCTOR^BOB^T^^DR|||||||ADM|A0|
    EOS

    str.tr("\n", "\r")
  end
end
